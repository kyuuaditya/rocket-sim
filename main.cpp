...
int main() {
    ...
    long double rocketDryMass = 6.0e5; // kg
    ...
    // -------------------------------------------------------------- State 1 ----------------------------------------------------------
    while (rocketVelocity < 15000) { // Run for 5 minutes
        ...
        if (visual) {
            if (count % 20 == 0 && window.isOpen()) { // Update every 20 instances for smoother animation
                ...
                text_rocket_time.setString("Rocket Time: " + std::to_string(timeExpended * timeStep) + " seconds");
    ...
    while (distanceCruised + 5000 * 1e3 < distanceToCruise && cruiseVelocity > 0) { // 30 hours for reference
        sf::Event event; // event catcher to close window
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                window.close();
    ...
    while (distancetoDecelerate - landing_range > distanceDecelerated && x < d && landing_phrase) {
    ...
    timeExpended *= timeStep; // normalize timeExpended to seconds
